...
八条验收项**每条都有同名测试**（`TestAcceptance1_…` ~ `TestAcceptance8_…`）。
抽查了断言强度，不是走过场：

> ★ **2026-09-08 更正**：第 1 轮记录里写的「全部实跑通过」**是错的**。
> 这八个测试都依赖 `127.0.0.1:33306` 的 MySQL，验收机没有，**实际全部 SKIP**。
> 当时我的 grep 过滤掉了 `--- SKIP` 行，只看到结尾的 `PASS` / `ok` 就下了结论。
> 下表的断言强度分析仍然成立（代码是对的），但**这八条从未被观察到真的跑过**。
> 补跑方案见 P2-12。

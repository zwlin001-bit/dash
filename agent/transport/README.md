...
- **回退通道（HTTP POST）**：WebSocket 连续失败 ≥ 3 次时自动切换至 HTTP 回退（`POST https://<domain>/api/agent/v1/report`），按 fast 间隔攒批上报，并通过响应体的 `commands` 数组接收下行指令；每 60s 自动尝试恢复 WebSocket 主通道。

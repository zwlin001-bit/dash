...
func TestDiskCollector(t *testing.T) {
	...
	// 模拟 Statfs 返回
	c.Statfs = func(path string) (total, used int64, err error) {
		switch path {
		case "/":
			return 40000000000, 10000000000, nil
		case "/data":
			return 60000000000, 20000000000, nil
		default:
			return 0, 0, nil
		}
	}
...
func TestConnsCollector(t *testing.T) {
	...
	if *sample.Slow.TCPCount != 3 {
		t.Errorf("expected TCPCount 3, got %d", *sample.Slow.TCPCount)
	}
	// udp (1) + udp6 (0) = 1
	if *sample.Slow.UDPCount != 1 {
		t.Errorf("expected UDPCount 1, got %d", *sample.Slow.UDPCount)
	}
}
...
func TestFactsCollector(t *testing.T) {
	...
	c.Statfs = func(path string) (total, used int64, err error) {
		if path == "/" {
			return 42000000000, 10000000000, nil
		}
		return 0, 0, nil
	}
	c.IPLookup = func() (string, string) {
		return "198.51.100.1", "2001:db8::1"
	}
